...
public class Body implements Iterable<Atom> {
        ...
        for (int i = 0; i < this.literals.size(); i++) {
          ...
          if (!atom1.getRelation().equals(atom2.getRelation())) {
            ...
            if (!checkValuesAndVariables(
                variablesThis2That, variablesThat2This, atom1, atom2, true)) return false;
            if (!checkValuesAndVariables(
                variablesThis2That, variablesThat2This, atom1, atom2, false)) return false;
          }
        }
        return true;
      }
  ...
  private boolean checkValuesAndVariables(
      HashMap<String, String> variablesThis2That,
      HashMap<String, String> variablesThat2This,
      Atom atom1,
      Atom atom2,
    ...
    if (!atom1.isLRC(leftNotRight) && !atom2.isLRC(leftNotRight)) {
      ...
      if (atom1.getLR(leftNotRight).equals("X") && !atom2.getLR(leftNotRight).equals("X"))
        return false;
      if (atom2.getLR(leftNotRight).equals("X") && !atom1.getLR(leftNotRight).equals("X"))
        return false;

      if (atom1.getLR(leftNotRight).equals("Y") && !atom2.getLR(leftNotRight).equals("Y"))
        return false;
      if (atom2.getLR(leftNotRight).equals("Y") && !atom1.getLR(leftNotRight).equals("Y"))
        return false;
